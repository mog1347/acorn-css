...
If  ```namespace``` is not empty string , it will be added to all the css classes as a prefix. If ```namespace``` is "k", class such as ```.text-center``` will change to ```.k-text-center```. The class names mentioned below will supposed without namespace.

#### useImportant

**Def:** Whether add ```!important``` to all css classes or not
**Value:** {Boolean}
**Default:** false
...
**Def:** Whether add ```!important``` to display classes or not. The builtin display class names begin with ```d-*```
**Value:** {Boolean}
**Default:** false
